...
class ButtonStatus extends EventEmitter {
  constructor(mapping, packetByteLength = 8) {
    ...
    // Mapping says logical board button 0 is like hardware 36 (mapping[0] === 36)
    this.mapLogicalToHw = mapping;
    this.mapHwToLogical = invertArray(mapping);
  ...
  set(buttonNum, value) {
    if (buttonNum >= this.state.length) return;
  ...
  toJSON() {
    let mappedStates = { buttonCount: 0, buttons: {} };
    for (let buttonIdx = 0; buttonIdx < this.state.length; buttonIdx++) {
      ...
      mappedStates.buttons[buttonIdx] = this.isPressed(buttonIdx);
      mappedStates.buttonCount++;
    }
    return mappedStates;
  }
